...
##############################################################
class If(ast.AST):
    _fields = ["ops", "comparators", "left", "body"]

    def __init__(self, ops, comparators, left, body):
        self.ops = ops
        self.body = body
...
#  
##############################################################
primops = {ast.Add: "add#", ast.Mult: "mult#", ast.Sub: "sub#", ast.Div: "div#"}

primcomops = {"Lt" : lc.ICMP_SLT, "Gt": lc.ICMP_SGT, "Eq": lc.ICMP_NE, "NotEq": lc.ICMP_EQ, "LtE": lc.ICMP_SLE, "GtE": lc.ICMP_SGE}
...
class TypeInfer(object):
    ...
    def visit_If(self, node):
        ...
        self.constraints += [(comparators, int32), (left, int32)]
        map(self.visit, node.body)
...
class PythonVisitor(ast.NodeVisitor):
    ...
    def visit_If(self, node):
        stmts = map(self.visit, node.body)
        ops = type(node.test.ops[0]).__name__
        comparators = self.visit(node.test.comparators[0])
        left = self.visit(node.test.left)
        
        return If(ops, comparators, left, stmts)
    ...
    def visit_AugAssign(self, node):
        ...
        if isinstance(node.op, ast.Sub):
            ref = node.target.id
            value = self.visit(node.value)
            return Assign(ref, Prim("sub#", [Var(ref), value]))
        if isinstance(node.op, ast.Div):
            ref = node.target.id
            value = self.visit(node.value)
            return Assign(ref, Prim("div#", [Var(ref), value]))
        else:
            raise NotImplementedError
...
class LLVMEmitter(object):
    ...
    def visit_If(self, node):
        ...
        init_block = self.function.append_basic_block('if.init')
        body_block = self.function.append_basic_block('if.body')
        ...
        cond = self.builder.icmp(primcomops[node.ops], left, comparators)
        self.builder.cbranch(cond, body_block, end_block)
